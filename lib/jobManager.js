...
let u2 = require('util');
const eh = require('beamhashverify');
...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.setForkHeights = function (fork1, fork2) {
    	_this.forkheight1 = fork1;
    	_this.forkheight2 = fork2;
    }
	...
	if (this.currentJob.rpcData.height != job.rpcData.height) {
        ...
        let powType=0;									// Beam Hash I
    
        if ((_this.forkHeight1 > 0) && (job.rpcData.height >= _this.forkHeight1)) {  // Beam Hash II
        	powType=1;
        }  
        if ((_this.forkHeight2 > 0) && (job.rpcData.height >= _this.forkHeight2)) {	// Beam Hash III
        	powType=2;
        }   
        ...
        const isValid = bh.verify(inputBuf, nonceBuf, outputBuf, powType);
