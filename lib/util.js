...
exports.beamDiffUnpack = function (packedDiff) {
    const leadingBit = 1 << 24;
    const order = packedDiff >> 24;
    let result = (leadingBit | (packedDiff & leadingBit - 1)) * Math.pow(2, order - 24);
    return Math.abs(result);
}

// BEAM
exports.getMiningReward = function(h) {
        ...
        const Drop1 = 1440 * 365 * 4; // 4 years roughly. Each such a cycle there's a new drop
        let base = 80, Value0 = 80;
        let hEnd;
        h -= 1;
                ...
                let n = 1 + (h - Drop0) / Drop1;
                hEnd = 1 + Drop0 + n * Drop1;
                if (n >= 2)
                        base += (base >> 2); // the unusual part - add 1/4
                return base >> n;
}

exports.beamDiffPack = function(unpackedDiff) {
 const bits = 32 - Math.clz32(unpackedDiff)
 let correctedOrder = bits-24-1
 let mantissa = unpackedDiff * Math.pow(2, -correctedOrder) - Math.pow(2, 24);
 let order = 24 + correctedOrder
