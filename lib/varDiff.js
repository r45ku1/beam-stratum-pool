...
var varDiff = module.exports = function varDiff(port, varDiffOptions) {
    ...
    this.manageClient = function (client) {
        ...
        client.on('submit', function () {
            ...
            if (avg > tMax && client.userSetDifficulty > options.minDiff) {
                if (options.x2mode) {
                    ddiff = 0.5;
                }
                if (ddiff * client.userSetDifficulty < options.minDiff) {
                    ddiff = options.minDiff / client.userSetDifficulty;
                }
            } else if (avg < tMin) {
                if (options.x2mode) {
                    ddiff = 2;
                ...
                if (ddiff * client.userSetDifficulty > diffMax) {
                    ddiff = diffMax / client.userSetDifficulty;
                }
            }
            else {
                return;
            ...
            var newDiff = toFixed(client.userSetDifficulty * ddiff, 8);
            timeBuffer.clear();
